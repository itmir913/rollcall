...
- **파서는 둘이다.** exceljs로 먼저 읽고, 실패하면 SheetJS로 다시 읽는다. 한셀·나이스가
  내보낸 xlsx는 규격에서 조금씩 벗어나 exceljs가 거부하는 경우가 있다. 어느 파서가
  읽었는지 화면에 알린다 — 값이 이상할 때 어디를 의심할지 알려주는 단서다.
- SheetJS는 npm 저장소에 없다. 별도 배포처의 tarball을 `vendor/`에 두고
  `file:vendor/...`로 참조한다. npm의 `xlsx`는 옛 버전이라 쓰지 않는다.
- **버린 줄은 조용히 넘기지 않는다.** 30명 중 29명만 들어왔는데 아무 말이 없으면
  교사는 알 방법이 없다. 몇 번째 줄이 왜 빠졌는지 함께 보여준다.
- CSV는 CP949(euc-kr)일 수 있다. UTF-8로만 읽으면 이름이 전부 깨진 채 들어간다.
...
- 엑셀·CSV 열을 인덱스로 읽기 (별칭표로 이름을 맞춘다)
